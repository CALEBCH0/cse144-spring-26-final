...
def build_transforms(model, img_size: int | None, aug_cfg: dict, train: bool,
                     crop_pct: float | None = None) -> Callable:
    ...
    Eval:  resize -> center-crop at img_size. Pass crop_pct to override the model
    default (used by multi-scale TTA: crop_pct = base_crop_pct / scale).
    """
    ...
    effective_crop_pct = crop_pct if crop_pct is not None else data_cfg.get("crop_pct", 0.875)
    return create_transform(
        input_size=data_cfg["input_size"],
        is_training=False,
        mean=data_cfg["mean"],
        std=data_cfg["std"],
        crop_pct=effective_crop_pct,
        interpolation=data_cfg.get("interpolation", "bicubic"),
    )
